    ...
    def _show_update_notification(self, new_version, download_url, asset_url=None):
        def show():
            ...
            if asset_url:
                ...
                self.status_var.set(self._t("downloading_update") if self.lang == "zh" else "Downloading update...")
                self.root.update()
    ...
    def show_help(self):
        txt = {
            "zh": (f"GanttPilot v{VERSION} - 协作式项目管理器\n\n"
                   "右键点击树状图进行所有操作：\n"
                   "• 空白处右键 → 添加项目 / 同步 / 刷新\n"
                   "• 项目右键 → 添加里程碑 / 生成报告 / 同步 / 删除\n"
                   "• 里程碑右键 → 添加计划 / 编辑描述 / 设置颜色 / 删除\n"
                   "• 计划右键 → 添加活动 / 编辑属性 / 设置颜色 / 终结 / 删除\n"
                   "• 活动右键 → 删除\n\n"
            ...
            "en": (f"GanttPilot v{VERSION} - Collaborative Project Manager\n\n"
                   ...
                   "• Empty area → Add project / Sync / Refresh\n"
                   "• Project → Add milestone / Report / Sync / Delete\n"
                   "• Milestone → Add plan / Edit desc / Color / Delete\n"
                   "• Plan → Add activity / Edit / Color / Finish / Delete\n"
                   "• Activity → Delete\n\n"
                   f"GitHub: https://github.com/{GITHUB_REPO}"),
        }
