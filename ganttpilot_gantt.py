...
class GanttRenderer:
    ...
    def draw(self):
        ...
        # Collect tasks
        tasks = []  # (type, label, start, end, status, executor, plan_color, ms_color, deadline, progress)
        for ms in self.project.get("milestones", []):
            ms_color = ms.get("color", "")
            ms_deadline = _parse_date(ms.get("deadline", ""))
            tasks.append(("milestone", ms["name"], None, None, None, "", "", ms_color, ms_deadline, 0))
            for plan in ms.get("plans", []):
                ...
                tasks.append(("task", label, start, end, status, executor, plan_color, ms_color, None, progress))
        ...
        # ── Draw tasks ───────────────────────────────────────
        for row, (kind, label, start, end, status, executor, plan_color, ms_color, deadline, progress) in enumerate(tasks):
